// app/(routes)/BreedDetection/ClientBreedDetection.jsx
"use client";
...
export default function ClientBreedDetection() {
  ...
  // HANDLE SUBMIT WITH 30s TIMEOUT
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data?.quality?.status === "ok") {
        ...
        toast.warning("Image quality is poor. Please upload a clearer image.");
        setPreview(null);
      }
    } catch (err) {
      // ✅ TIMEOUT HANDLING
      if (err.name === 'AbortError') {
        toast.error("Prediction taking too long (30s timeout). Please try again.");
      } else {
